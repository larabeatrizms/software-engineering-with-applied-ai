...
export default class Events {
    ...
    static onLikedAdded(callback) {
        document.addEventListener(events.likedAdded, (event) => {
            return callback(event.detail);
        });
    }
    static dispatchLikedAdded(data) {
        const event = new CustomEvent(events.likedAdded, {
            detail: data
        });
        document.dispatchEvent(event);
    }

    static onLikedRemoved(callback) {
        document.addEventListener(events.likedRemoved, (event) => {
            return callback(event.detail);
        });
    }

    static dispatchLikedRemoved(data) {
        const event = new CustomEvent(events.likedRemoved, {
            detail: data
        });
        document.dispatchEvent(event);
    }
