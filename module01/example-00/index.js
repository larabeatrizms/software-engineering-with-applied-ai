...
async function trainModel(inputXs, outputYs) {
    ...
    // inputShape 7 pois são 7 camadas, 7 posições de array
    // units 80 é a quantidade de neuronios
    // a relu age como um filtro
    model.add(tf.layers.dense({ inputShape: [7], units: 80, activation: 'relu' }))

    // Saída: 3 neuronios
    // Um para cada categoria (premium, meduim, basic)
    ...
    // maior o erro (loss)
    // Exemplo classico: classificação de imagens, recomendação, categoriação de usuário
    // qualquer coisa em que a resposta certa é apenas uma entre várias possíveis

    model.compile({ optimizer: 'adam', loss: 'categoricalCrossentropy', metrics: ['accuracy'] })

    // verbose desabilita log interno
    // epoch: quantidade de vzes que vai rodar no dataset
